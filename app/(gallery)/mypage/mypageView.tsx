...
const MypageView :React.FC<MypageViewProps> = ({studentarr,scoredata}) => {
    ...
    // 1日目の評価された人数
    const day1arr = currentData.filter((v)=> v.date == "2023-12-12");
    const day1 = day1arr.length;
    // 2日目の評価された人数
    const day2arr = currentData.filter((v)=> v.date == "2023-12-14");
    const day2 = day2arr.length;
    // 3日目の評価された人数
    const day3arr = currentData.filter((v)=> v.date == "2023-12-15");
    ...
    // 2日目
    day2arr.map((data)=>{
        sum[2] = sum[1] + data.score_1 + data.score_2 + data.score_3 + 100;
    })
    // 3日目
    day3arr.map((data)=>{
        sum[3] = sum[1] + data.score_1 + data.score_2 + data.score_3 + 100;
    })
